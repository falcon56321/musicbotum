  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      if (stp === "normal") {
        ...
        await interaction.reply({ content: `🔍 **${lang.msg61}** \`${name}\`` }).catch(e => { })
        
        try {
          // Eğer URL ise doğrudan kullan, değilse YouTube araması olarak işaretle
          let searchTerm = name;
          
          // URL olup olmadığını kontrol et
          if (!searchTerm.startsWith('http://') && !searchTerm.startsWith('https://')) {
            searchTerm = `ytsearch:${encodeURIComponent(searchTerm)}`;
          }
          
          console.log("Çalınacak içerik:", searchTerm);
      ...
      console.error("Play command global error:", e);
